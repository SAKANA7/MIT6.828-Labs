...
page_init(void)
{
	...
	for (i = 1; i < npages; i++) {
		...
		//Mark the rest as unused free space.
		else
		{
			pages[i].pp_ref = 0;
...
page_free(struct PageInfo *pp)
{
	...
	//Link page pp back into the free physical pages list
	...
	{// If the entry is not present
		if(create)
		{
			...
			else
			{
				...
				pte_t* page_table = page2kva(tmp);   //convert physical page to virtual address
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	*p_pte = page2pa(pp) | perm |PTE_P; //Set the page table entry
	tlb_invalidate(pgdir , va); //invalidate TLB because of change occurs
	return 0;
}
...
//   - 4. The TLB must be invalidated if you remove an entry from
//     the page table.
//
